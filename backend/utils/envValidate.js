...
function envValidate() {
  ...
  for (const env in envList) {
    const ec = process.env[env];
    if (!ec && envList[env].required) {
      console.error(`Не задана обязательная переменная ${env}`);
      success = false;
    } else if (!ec && !envList[env].required) {
      console.warn(
        `Не задана опциональная переменная ${env}. Используется значение по умолчанию: ${envList[env].default}`
      );
  ...
  if (!success) {
    console.error(
      "Валидация переменных окружения не удалась! Проверьте файл .env.tpl и на его основе сделайте файл .env"
    );
    process.exit(1);
  } else {
